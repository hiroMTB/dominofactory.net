...
/*     gl.depthFunc(gl.LEQUAL); */
  ...
  $('html, body').css("background-color","white");
  	...
  	// main rect
   changeColor();
   setInterval(changeColor, 1121);
   setInterval(changeColor, 3516);
   setInterval(changeColor, 5192);
   setInterval(changeColor, 8192);
   
   // background
  if(Math.random()>0.5){
	  changeBackgoundColorRandom();
	  setInterval(changeBackgoundColorRandom, 5000);
  }
}
...
function changeBackgoundColorRandom(){
	...
	var time = Math.random()*10000 + 20000;
...
function animateColor(){
	for(var i=0; i<16; i++){
		colors[i] *= 1000.0;
 		colors[i] = colors[i]*0.99 + targetColors[i]*1000.0*0.01;
 		colors[i] *= 0.001;
	}
